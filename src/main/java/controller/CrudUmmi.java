...
public class CrudUmmi {
    ...
    private void tambahSantri() {
        ...
        while (true) {
            ...
            System.out.println("Nomor telepon harus berupa angka sebanyak 10-15 digit");
        }
        ...
        while (true) {
            ...
            KelasMengaji kelasDipilih = cariKelasBerdasarkanKode(pilihanKelas);
            ...
            if (jumlahSantriDalamKelas(kelasDipilih)  >= kelasDipilih.getKapasitas()) {
        ...
        private void updateKelasSantri() {
            ...
            while (true) {
                ...
                KelasMengaji kelasBaru = cariKelasBerdasarkanKode(kodeKelas);
                ...
                if (kelasLama.getKodeKelas()== kelasBaru.getKodeKelas()) {
                    System.out.println("Santri sudah berada di kelas tersebut");
                    return;
                }

                if (jumlahSantriDalamKelas(kelasBaru) >= kelasBaru.getKapasitas()) {
        ...
        private int jumlahSantriDalamKelas(KelasMengaji kelas) {
            ...
            for (Santri santri : daftarSantri) {
                if (santri.getKelasMengaji().getKodeKelas() == kelas.getKodeKelas()) {
                    jumlah++;
                }
            }
            return jumlah;
    ...
    private void updateKelasMengaji() {
        ...
        while (true) {
            ...
            KelasMengaji kelasDipilih = cariKelasBerdasarkanKode(kodeKelas);
            ...
            while (true) {
                ...
                if (kapasitas < jumlahSantriDalamKelas(kelasDipilih)) {
                    System.out.println("Kapasitas tidak boleh lebih kecil dari jumlah santri saat ini");
                    continue;
                }
                break;
            }
    ...
    private void hapusKelasMengaji() {
        ...
        while (true) {
            ...
            if (jumlahSantriDalamKelas(kelasDipilih) > 0) {
                System.out.println("Kelas tidak dapat dihapus karena masih memiliki santri");
                return;
            }
    ...
    private void tambahPengajar() {
        ...
        while (true) {
            mengajar = menu.bacaTeks("Kelas Mengajar: ");

            if (!mengajar.isBlank()) {
                break;
            }
            menu.tampilkanPesan("Mengajar tidak boleh kosong");
        }
        daftarPengajar.add(new Pengajar(nama,nomorTelepon,mengajar));
    ...
    private void updatePengajar() {
        ...
        while (true) {
            mengajarBaru = menu.bacaTeks("Masukkan kelas mengajar baru: ");

            if (!mengajarBaru.isBlank()) {
                break;
            }
            menu.tampilkanPesan("Mengajar tidak boleh kosong");
        }

        Pengajar pengajarDipilih = daftarPengajar.get(nomor - 1);

        pengajarDipilih.setMengajar(mengajarBaru);
        menu.tampilkanPesan("Kelas Mengajar berhasil diupdate");
    }
